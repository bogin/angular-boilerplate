...
export class CellsArray {
  ...
  initEmptyCells = (): void => {
    this._cells = [...Array(this.board.rows)].map((item: any, row: number) =>
      this.initEmptyBoardRow(row, this.board.columns!)
    );
  };

  private initEmptyBoardRow = (row: number, columnSize: number) => {
    return [...Array(columnSize)].map((item: any, column: number) =>
      this.getEmptyCell(row, column)
    );
  };
  ...
  initCellsWithBombs = (cellWithoutBomb?: BoardCell): void => {
    ...
    while (totalMines! > 0) {
      const row = Math.floor(Math.random() * (this.board.rows || 0));
      const column = Math.floor(Math.random() * (this.board.columns || 0));
  ...
  private shouldPlaceBomb = (
    row: number,
    column: number,
    cellWithoutBomb?: BoardCell
  ): boolean => {
    const initAfterFirstClick = cellWithoutBomb;
    const bombIndexIsEquelToFirstClickIndex =
      row === cellWithoutBomb?.row && column === cellWithoutBomb.column;
    return (
      this.cells[row][column].type !== CellType.Bomb &&
